# Project Memories — Rust-ifying Causal-Inference Tooling for Epidemiology

*Saved memory entries for this project (as of June 29, 2026).*

1. User is working on a project to modernize causal inference / epidemiology tooling for healthcare research by porting performance-critical parts of R epi packages to Rust, using Claude Code as an autonomous (fixture-driven) coding loop.
...
5. tte-expand v1 scope is sequential trial expansion **ONLY** (Hernán 2008 / Gran 2010 / Danaei 2013) — not the clone-censor-weight grace-period design (which is upstream TrialEmulation issue #115, a separate later contribution) and not a statistical solver. Gemini had conflated sequential TE with CCW; user is aware of the distinction.

6. Contribution plan for tte-expand: ship as a companion extendr R package, produce a computational-reproducibility certificate (bit-exact equivalence to the CRAN gold standard), engage the maintainers early, and write up via JOSS / preprint with an ENCePP / RWE-reproducibility framing. Positioning is 'verified high-performance backend with the maintainers', not a replacement.
